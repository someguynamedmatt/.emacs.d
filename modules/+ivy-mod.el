...
Buffers that are considered unreal (see `my-real-buffer-p') are dimmed with
  ...
  (let ((this-command 'counsel-find-file))
    (call-interactively
     (cond ((or (file-equal-p default-directory "~")
                ...
                (when-let (proot (my-project-root))
                  (file-equal-p proot "~")))
            #'counsel-find-file)
...
:recursive BOOL
  ...
  (let* ((this-command 'counsel-rg)
         (project-root (or (my-project-root) default-directory))
